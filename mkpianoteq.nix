...
let
  ...
  filename = {
    "5" = "pianoteq_linux_v${major}${minor}${patch}.7z";
    "6" = "pianoteq_linux_v${major}${minor}${patch}.7z";
    "7" = "pianoteq_linux_v${major}${minor}${patch}.7z";
    "8" = "pianoteq_linux_v${major}${minor}${patch}.7z";
    "9" = "pianoteq_setup_v${major}${minor}${patch}.tar.xz";
  }.${major};
  icon = fetchurl
    {
...
stdenv.mkDerivation
{
  ...
  src = requireFile {
    name = filename;
    message = "Download the file from: https://www.modartt.com/download?file=${filename} and add it to the nix store manually: nix-store --add-fixed sha256 ./${filename}";
    inherit hash;
  ...
  installPhase = ''
    install -Dm 755 x86-64bit/Pianoteq\ ${major} $out/bin/pianoteq${major}
    install -Dm 755 x86-64bit/Pianoteq\ ${major}.lv2/Pianoteq_${major}.so \
                    $out/lib/lv2/Pianoteq\ ${major}.lv2/Pianoteq_${major}.so
    patchelf --set-interpreter "$(< $NIX_CC/nix-support/dynamic-linker)" \
             --set-rpath $libPath "$out/bin/pianoteq${major}"
    cd x86-64bit/Pianoteq\ ${major}.lv2/
    for i in *.ttl; do
        install -D "$i" "$out/lib/lv2/Pianoteq ${major}.lv2/$i"
    done
    runHook postInstall
  '';
